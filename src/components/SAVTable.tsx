...
export const SAVTable = ({ savRecords, weekStart, isAbove, onTogglePosition, isAdmin }: SAVTableProps) => {
  ...
  const handleResolvedChange = async (id: string, checked: boolean) => {
    ...
    try {
      await updateResolved.mutateAsync({ 
        id, 
        est_resolu: checked, 
        weekStart 
      });
  ...
  // Remove ", France" from address for display
  const formatAddress = (address: string) => {
    return address.replace(/,\s*France$/i, '').trim();
  };

  // Get first phone number for display, full content for tooltip
  ...
  return (
    <Card className={cn(
      ...
      <CardHeader className="bg-orange-50 dark:bg-orange-950/30 border-b py-2 px-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <CardTitle className="text-base font-semibold flex items-center gap-2 text-orange-700 dark:text-orange-300">
              ...
              Service Après-Vente ({filteredAndSortedRecords.length})
            </CardTitle>
            <div className="flex items-center gap-1 bg-white/50 dark:bg-black/20 rounded-md p-0.5">
              <Button
              ...
              <Button
                variant={filterStatus === 'unresolved' ? 'secondary' : 'ghost'}
                size="sm"
      ...
      <CardContent className="p-0 max-h-[300px] overflow-y-auto">
        <Table className="table-fixed w-full">
          <TableHeader className="sticky top-0 bg-muted/80 backdrop-blur-sm z-10">
            <TableRow>
              <TableHead 
                className="w-[5%] text-center cursor-pointer hover:bg-muted/50 transition-colors"
              ...
              <TableHead 
                className="w-[10%] whitespace-nowrap cursor-pointer hover:bg-muted/50 transition-colors"
              ...
              <TableHead 
                className="w-[5%] text-center cursor-pointer hover:bg-muted/50 transition-colors"
                onClick={() => handleSort('est_resolu')}
              >
                <span className="flex items-center justify-center">
                  Résolu<SortIcon field="est_resolu" />
                </span>
              </TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {filteredAndSortedRecords.map((record) => (
              <TableRow 
                ...
                className={cn(
                  "transition-colors",
                  record.est_resolu && "bg-green-50 dark:bg-green-950/20 text-muted-foreground"
                )}
              >
